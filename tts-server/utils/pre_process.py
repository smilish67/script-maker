...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        json_files_path = f'../dataset/VL_' + audio_num
        audio_files_path = f'../dataset/VS_' + audio_num
        ...
        for i, audio_file in enumerate(audio_files):
            ...
            os.rename(audio_files_path + '/' + audio_file, f'../dataset/actors/{audio_num}' + '/'+ audio_file.split('.')[0][2:4] +'/wavs/V' + audio_file_name)
        ...
        #save all text csv
        for key in all_text:
            with open(f'../dataset/actors/{audio_num}/{key}/metadataVal.csv', 'w', encoding='utf-8') as f:
                for i in range(len(all_text[key])):
                    # 4 digit number
                    f.write('V'+key + f'{i+first_idx:04d}' + '|' + all_text[key][i][0] +'|'+ all_text[key][i][1]+ '\n')
                f.close()
