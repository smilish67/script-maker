...
def json_formatter(files, pre_path, actor_num, type):
    ...
    for json_file in files:
        ...
        with open(pre_path + '/' + json_file, 'r', encoding='utf-8') as f:
            ...
            all_text[emotional_type].extend(data[0]['script']['normalized'])
            f.close()
    ...
    for key in all_text:
        with open(f'../dataset/metadata{type}.csv', 'a', encoding='utf-8') as f:
            for i in range(len(all_text[key])):
                # 4 digit number
                if type == 'T':
                    f.write(type + '_' + key + '_'+ actor_num + '_'+ f'{i+101:04d}' + '|' + all_text[key][i] + '|' + actor_num + '|' + key +'\n')
                else:
                    f.write(type + '_' + key + '_'+ actor_num + '_'+ f'{i+51:04d}' + '|' + all_text[key][i] + '|' + actor_num + '|' + key +'\n')
            f.close()
